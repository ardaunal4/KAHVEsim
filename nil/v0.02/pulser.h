...
public:
    ...
    void applyPulse(const std::vector<EFieldPoint>& fieldPoints, float time, float* outputFields, int numParticles, const Particle* particles) {
        for (int i = 0; i < numParticles; ++i) {
            float delay = calculateDelay(particles[i].z);
